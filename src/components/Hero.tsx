  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          backgroundImage:
            'radial-gradient(circle at 20% 80%, #0ea5e9 0%, transparent 50%), radial-gradient(circle at 80% 20%, #14b8a6 0%, transparent 50%)',
        }}
      />
      ...
      <div className="relative z-10 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 text-center">
        <motion.div
          ...
          transition={{ duration: 0.7, delay: 0.2 }}
        >
          <span className="inline-block px-3 py-1.5 sm:px-4 bg-sky-500/20 border border-sky-400/30 text-sky-300 text-[10px] sm:text-xs font-semibold tracking-widest uppercase rounded-full mb-6 backdrop-blur-sm">
            Kakinada, Andhra Pradesh
          </span>
        </motion.div>
        ...
        <motion.h1
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, delay: 0.35 }}
          className="text-3xl min-[400px]:text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-extrabold text-white tracking-tight mb-4 px-2 leading-tight"
        >
          HK{' '}
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-300 to-teal-300">
            EXPORTS
        ...
        <motion.p
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, delay: 0.5 }}
          className="text-base min-[400px]:text-lg sm:text-xl md:text-2xl text-sky-100/90 font-light mb-4 max-w-2xl mx-auto leading-relaxed px-2"
        >
          Fresh Fish &amp; Prawns Delivered Across India
        </motion.p>
        ...
        <motion.p
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, delay: 0.6 }}
          className="text-sm sm:text-base text-slate-300/80 mb-8 sm:mb-10 max-w-lg mx-auto px-4"
        >
          Sourced from the pristine waters of the Bay of Bengal — farm fresh,
          hygienically processed, and swiftly delivered.
        </motion.p>
        ...
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, delay: 0.7 }}
          className="flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-3 sm:gap-4 w-full max-w-md mx-auto"
        >
          <button
            onClick={() => scrollTo('#contact')}
            className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 sm:px-8 py-3.5 sm:py-4 bg-gradient-to-r from-sky-500 to-teal-500 text-white font-semibold rounded-xl hover:shadow-2xl hover:shadow-sky-500/30 hover:-translate-y-1 transition-all duration-300 text-sm sm:text-base"
          >
            <MessageCircle className="w-5 h-5" />
            Contact Us
          </button>
          ...
          <button
            onClick={() => scrollTo('#products')}
            className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 sm:px-8 py-3.5 sm:py-4 bg-white/10 backdrop-blur-sm text-white font-semibold rounded-xl border border-white/25 hover:bg-white/20 hover:-translate-y-1 transition-all duration-300 text-sm sm:text-base"
          >
            <ShoppingBag className="w-5 h-5" />
            View Products
          </button>
        ...
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, delay: 0.9 }}
          className="mt-10 sm:mt-16 flex flex-col sm:flex-row items-center justify-center gap-5 sm:gap-10 lg:gap-16 px-4"
        >
          {[
            { value: '100%', label: 'Farm Fresh' },
            { value: 'Pan India', label: 'Delivery' },
            ...
            <div key={stat.label} className="text-center">
              ...
              <p className="text-sky-300/80 text-xs sm:text-sm font-medium mt-0.5">
                {stat.label}
              </p>
            </div>
          ))}
